...
export const statsCommand = new Command("stats")
  ...
  .action(() => {
    ...
    Object.entries(stats.byStatus).forEach(([status, count]) => {
      ...
      console.log(
        `  ${color(status.padEnd(6))} ${count.toString().padStart(3)} ${color(bar)}`,
      );
    });
    ...
    Object.entries(stats.byPriority).forEach(([priority, count]) => {
      ...
      const bar = "█".repeat(Math.round((count / stats.total) * 20) || 0);
      console.log(
        `  ${color(priority.padEnd(7))} ${count.toString().padStart(3)} ${color(bar)}`,
      );
    });
    ...
    if (tagEntries.length > 0) {
      ...
      tagEntries
        ...
        .forEach(([tag, count]) => {
          const bar = "█".repeat(Math.round((count / stats.total) * 20) || 0);
          console.log(
            `  ${chalk.cyan(tag.padEnd(12))} ${count.toString().padStart(3)} ${chalk.cyan(bar)}`,
          );
        });
      console.log();
    }
